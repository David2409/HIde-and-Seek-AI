...
public class Connection extends Thread{
	...
	public boolean ExecuteCommand(String[] input){
		try {
			switch(input[0]){
			case "START":
				//			  field 1 							| gool 2 							            | cockies 3 						  | 					playerPos 4,5 					  |   		maxMoves 6 		  | 		net amount 7 	 | 		  ressistance 8 	  |         calculateIn		 |				OverworkData
				server.StartT(XML.StirngToFloatArray2(input[1]).array, XML.StirngToFloatArray2(input[2]).array, XML.StringToIntArray2(input[3]).array, Integer.parseInt(input[4]), Integer.parseInt(input[5]), Integer.parseInt(input[6]), Integer.parseInt(input[7]), Integer.parseInt(input[8]), Integer.parseInt(input[9]), XML.StringToOverworkDataArray(input[10]).array);
				break;
			case "STOP":
				server.StopT();
				break;
